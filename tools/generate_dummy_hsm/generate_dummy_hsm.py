...
from yubihsm.objects import YhsmObject, ObjectInfo, AsymmetricKey, AuthenticationKey, Opaque
...
def arg_parser_builder():
    ...
    parser.add_argument('-f', '--cert_json_file',
                        type=argparse.FileType('r'),
                        default='dummy_certs_4bsn.json',
                        help="The json defines certificate chain")
    
    parser.add_argument('--toolreq',
                        default='1.1',
                        help="The json defines certificate chain")

    group = parser.add_mutually_exclusive_group()
    group.add_argument('--test_cert',
                        action='store_true',
                        help="Program test certificates to HSM")
    group.add_argument('--preprod_cert',
                        action='store_true',
                        help="Program preproduction certificates to HSM")
...
def main():
    ...
    with YubiHsm.connect(args.connector) as hsm,\
            hsm.create_session_derived(1, "password") as session:
        ...
        try:
            ...
            AuthenticationKey.put_derived(session, 5, "SIDEWALK", 5,
                                          CAPABILITY.GET_OPAQUE+CAPABILITY.SIGN_ECDSA+CAPABILITY.SIGN_EDDSA,
                                          CAPABILITY.NONE, args.pin)
            hsmInfo = {
                "ver":      3,
            ...
            Opaque.put(session, 0x0E, "HSM_INFO", 5,
                       CAPABILITY.NONE, ALGORITHM.OPAQUE_DATA, hsmInfoData)

            Opaque.put(session, 0x0F, "SW_CTL_CHAIN_DEPTH", 5,
                       CAPABILITY.NONE, ALGORITHM.OPAQUE_DATA, b'\x05')
                    ...
                    if element == 'priv':
                        if ca.startswith("DAK"):
                            if curve == 'ED25':
                                ...
                                private_key = ec.derive_private_key(int.from_bytes(data, 'big'), ec.SECP256R1())

                            AsymmetricKey.put(session, index, label, 5,
                                              CAPABILITY.SIGN_EDDSA+CAPABILITY.SIGN_ECDSA,
                                              private_key)
                    else:
                        Opaque.put(session, index, label, 5, CAPABILITY.NONE, ALGORITHM.OPAQUE_DATA, data)
                    index += 1
                index += 0x4
            base += 0x10

        print("Completed the programming of dummy keys for %s_DAK_%s with PIN=%s" %
